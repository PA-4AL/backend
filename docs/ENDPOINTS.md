...
> (endpoint ajouté, supprimé, route ou méthode renommée, DTO de requête ou de
> réponse modifié, code d'erreur changé) doit être répercutée ici **dans le même
> commit**. C'est le contrat lu par le frontend ; un écart ici est un bug côté
> client.
>
> **Source de vérité :** `src/main/kotlin/org/example/backend/controller/`
> et `.../model/*Dtos.kt`.
> **Dernière mise à jour :** 25/07/2026 (commit `728416e`, 25 endpoints)
...
| Règle | Portée |
|---|---|
| `permitAll` | `GET /api/tournaments/**` uniquement (rôle Visiteur, spec §3) |
...
|---|---|---|---|---|
| 1 | GET | `/api/tournaments` | public | 200 |
| 2 | GET | `/api/tournaments/{id}` | public | 200 / 404 |
| 3 | POST | `/api/tournaments` | JWT | 201 |
| 4 | GET | `/api/tournaments/{id}/bracket` | public | 200 |
| 5 | POST | `/api/tournaments/{id}/bracket/generate` | JWT | 200 |
| 6 | POST | `/api/matches/{matchId}/score` | JWT | 200 |
| 7 | GET | `/api/tournaments/{id}/participants` | public | 200 |
| 8 | POST | `/api/tournaments/{id}/register` | JWT | 201 |
| 9 | POST | `/api/tournaments/{id}/register-team` | JWT | 201 |
| 10 | POST | `/api/tournaments/{id}/participants` | JWT | 201 |
| 11 | GET | `/api/registrations/pending` | JWT | 200 |
| 12 | POST | `/api/registrations/{id}/seed` | JWT | 204 |
| 13 | POST | `/api/registrations/{id}/confirm` | JWT | 204 |
| 14 | POST | `/api/registrations/{id}/reject` | JWT | 204 |
| 15 | GET | `/api/me` | JWT | 200 |
| 16 | PATCH | `/api/me` | JWT | 200 |
| 17 | POST | `/api/me/game-accounts` | JWT | 201 |
| 18 | DELETE | `/api/me/game-accounts/{id}` | JWT | 204 |
| 19 | GET | `/api/teams/mine` | JWT | 200 |
| 20 | GET | `/api/teams/{id}` | JWT | 200 |
| 21 | POST | `/api/teams` | JWT | 201 |
| 22 | POST | `/api/teams/{id}/members` | JWT | 200 |
| 23 | DELETE | `/api/teams/{id}/members/{memberId}` | JWT | 200 |
| 24 | GET | `/api/dashboard/kpis` | JWT | 200 |
| 25 | GET | `/api/dashboard/activity` | JWT | 200 |
...
### 2. `GET /api/tournaments/{id}` — public

**200** → `TournamentDetailDto`
**404** → corps vide (pas de message)

### 3. `POST /api/tournaments` — JWT
...
**201** → `TournamentSummaryDto`, en-tête `Location: /api/tournaments/{id}`
...
### 6. `POST /api/matches/{matchId}/score` — JWT
...
### 7. `GET /api/tournaments/{id}/participants` — public

Triés par `seed` croissant (NULL en dernier) puis date d'inscription.

**200** → `ParticipantDto[]`

### 8. `POST /api/tournaments/{id}/register` — JWT
...
### 9. `POST /api/tournaments/{id}/register-team` — JWT
...
### 10. `POST /api/tournaments/{id}/participants` — JWT
...
### 11. `GET /api/registrations/pending` — JWT
...
### 12. `POST /api/registrations/{id}/seed` — JWT
...
### 17. `POST /api/me/game-accounts` — JWT
...
### 18. `DELETE /api/me/game-accounts/{id}` — JWT
...
### 22. `POST /api/teams/{id}/members` — JWT (capitaine)
...
### 23. `DELETE /api/teams/{id}/members/{memberId}` — JWT (capitaine)
...
### 25. `GET /api/dashboard/activity` — JWT
