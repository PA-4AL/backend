...
class TeamRepository(private val dsl: DSLContext) {
    ...
    fun listMine(userId: UUID): List<TeamRow> =
        dsl.select(TEAMS.ID, TEAMS.NAME, TEAMS.TAG)
            .from(TEAMS)
    ...
    fun find(teamId: UUID): TeamRow? =
        dsl.select(TEAMS.ID, TEAMS.NAME, TEAMS.TAG)
            .from(TEAMS)
            .where(TEAMS.ID.eq(teamId))
            .fetchOne { TeamRow(it.get(TEAMS.ID)!!, it.get(TEAMS.NAME)!!, it.get(TEAMS.TAG)) }

    fun members(teamId: UUID): List<MemberRow> =
        dsl.select(TEAM_MEMBERS.USER_ID, USERS.PSEUDO, TEAM_MEMBERS.ROLE)
            .from(TEAM_MEMBERS)
    ...
    fun isCaptain(teamId: UUID, userId: UUID): Boolean =
        dsl.fetchExists(
            TEAM_MEMBERS,
    ...
    fun findUserByPseudo(pseudo: String): UUID? =
        dsl.select(USERS.ID)
            .from(USERS)
    ...
    fun isMember(teamId: UUID, userId: UUID): Boolean =
        dsl.fetchExists(
            TEAM_MEMBERS,
